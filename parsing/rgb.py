...
class RGBController(Thread):
    ...
    def build() -> (object, None):
        """Attempt to build an RGBController if possible"""
        if variables.settings["realtime"]["rgb"] is False:
            return None
        try:
    ...
    def __init__(self):
        ...
        from rgbkeyboards import \
            Keyboards, KeyboardController, BaseKeyboard, \
            keygroups, effects
        self.keygroups, self.effects = keygroups, effects
        ...
        self._default = {key: (100, 100, 100) for key in keygroups.all}
    ...
    def process_item(self, item: tuple):
        ...
        if key in self.keygroups.pynput_rgb_keys:
            key = self.keygroups.pynput_rgb_keys[key]
        ...
        if "F" in key:
            ...
            effect = self.effects.build_breathe(self.RED, 3, self.keygroups.all, 0.05)
            ...
            trans = self.effects.build_transition(
                (100, 100, 100), self.FOREGROUND[key], 2, self.effects.ALL_KEYS, 0.05)
